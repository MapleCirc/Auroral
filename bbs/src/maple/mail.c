...
static void
catch_utility_terminate(int status)
{
  pid_t pid;
  ...
  int method;
{
  ...
  child_abort = 0;
  act.sa_handler = catch_utility_terminate;
  act.sa_flags = 0;
  sigemptyset(&act.sa_mask);
  sigaction(SIGCHLD, &act, &old_sa);

  pid = fork();
  switch(pid)
  {
  case 0:
    execl(BBSHOME"/bin/sendmail", NULL);
    exit(9);

  case -1:
    vmsg("錯誤：無法啟動寄信程序，請回報給站務");
    return -1;
  }

  sprintf(pipe_path, FN_TMP_SENDMAIL, pid);
  while(mkfifo(pipe_path, 0600) < 0)
  {
    if(errno != EEXIST && errno != EINTR)
    {
create_pipe_failed:
      vmsg("錯誤：無法建立通訊管線，請回報給站務");
      return -1;
    }
  ...
  if(child_abort == pid)
  {
start_sendmail_failed:
    vmsg("錯誤：啟動寄信程序失敗，請通知站務");
    unlink(pipe_path);
    return -1;
  }

  while(!(fp = fopen(pipe_path, "w")))
  {
    if(errno == EINTR)
    {
      if(child_abort == pid)
        goto start_sendmail_failed;
      else
        continue;
    }
    else
 ...
 *       還可以使在同一次認證時，所發出去的認證信標題都相同，避免有人老是不知道要回最後一封 */
  ...
  fwrite(&stamp, sizeof(time_t), 1, fp);	/* stamp */
  if(method & MQ_READONLY)		/* is_readonly */
    fwrite(&true_value, sizeof(int), 1, fp);
  else
    fwrite(&false_value, sizeof(int), 1, fp);
  if(method & MQ_NOATTACH)		/* no_attach */
    fwrite(&true_value, sizeof(int), 1, fp);
  else
    fwrite(&false_value, sizeof(int), 1, fp);
  fprintf(fp, "%s\n", fpath);   /* fpath */
  fprintf(fp, "%s\n", from);    /* from */
  fprintf(fp, "%s\n", rcpt);    /* rcpt */
  fprintf(fp, "%s\n", cuser.userid);    /* sender_userid */
  fprintf(fp, "%s\n", cuser.username);  /* sender_nick */
  fprintf(fp, "%s\n", title);   /* title */
  if(method & MQ_ATTACH)
  {
    fputs("application/x-compressed\n", fp);
    fputs("base64\n", fp);
    fprintf(fp, "%s.tgz\n", cuser.userid);
  }
  else
...
#ifdef EMAIL_JUSTIFY
  if(method & MQ_JUSTIFY)       /* is_justify */
    fwrite(&true_value, sizeof(int), 1, fp);
  else
    fwrite(&false_value, sizeof(int), 1, fp);
#endif
  ...
  int mode;
{
  ...
  sock = dns_smtp(userid);	/* itoc.註解: 這邊先試著連線，以免壓縮完才知道是無效的工作站地址 */
  if(sock >= 0)
  {
    close(sock);
    userid = cuser.userid;
    ...
    if(bsmtp(fpath, title, addr, MQ_ATTACH) < 0)
      unlink(fpath);
  }
}
  ...
  char *rcpt;
{
  ...
  else
  {
    ...
    if(vget(2, 0, "確定要寄出嗎(Y/N)？[Y] ", ans, sizeof(ans), LCECHO) == 'n')
    {
      vmsg(msg_cancel);
      return -1;
    }
    ...
    {
      mail_hold(fpath, rcpt, ve_title, 0);
      if(bsmtp(fpath, ve_title, rcpt, 0) < 0)
      {
	unlink(fpath);
	return -1;
      }
  ...
  XO *xo;
{
  ...
  for (;;)
  {
    ...
    if ((nmode = more(fpath, MFST_AUTO)) < 0)
      break;
  ...
  XO *xo;
{
  ...
  {
    if (HAS_PERM(PERM_ALLADMIN))	/* itoc.030914: 讓所有 ADMIN 都可以看 sysop/guest 信箱 */
    {
      char *userid;
      XO *xt;
      char fpath[64];
