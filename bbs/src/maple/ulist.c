...
extern UCACHE *ushm;
extern XZ xz[];
int idlemode = 1;	/* 1:Normal mode, 0:idle-only mode */
...
)
{
  ...
  ufo = up->ufo;
  if(cuser.ufo & UFO_IDLEONLY)	/* floatJ: 新增 麻吉 */
  {
    if(is_mymate(userno))
      outs("    \033[1;33m★\033[m");
    else
      prints("\033[1;30m%6d\033[m", num);
  }
  else
    prints("%6d", num);

  if(ufo & UFO_SUPERCLOAK)
    outc('>');
  else if(ufo & UFO_CLOAK)
    outc(')');
  else
    outc(' ');
...
#endif
  if(up->ufo & UFO_SUPERCLOAK)
    outs("\033[1;35m");
  else if(up->status & STATUS_BIRTHDAY)
    outs("\033[1;5;35m");
  else if(ftype & FTYPE_NORMAL)
    outs(COLOR_NORMAL);
  else if(ftype & FTYPE_BOTHGOOD)
    outs(COLOR_BOTHGOOD);
  else if(ftype & FTYPE_MYGOOD)
    outs(COLOR_MYGOOD);
  else if(ftype & FTYPE_OGOOD)
    outs(COLOR_OGOOD);
  else if(ftype & FTYPE_SELF)
    outs(COLOR_SELF);
  else if(ftype & FTYPE_MYBAD)
    outs(COLOR_MYBAD);

  prints("%-13s%-*.*s\033[m%-*.*s%-11.10s", up->userid,
    (d_cols >> 1) + 21, (d_cols >> 1) + 20, up->username,
    d_cols - (d_cols >> 1) + 19, d_cols - (d_cols >> 1) + 18,
    pickup_ship ? pal_ship(ftype, up->userno) :
#ifdef GUEST_WHERE
    /* 可看見 guest 的故鄉 */
    (!hide_from || sysop || (ftype & (FTYPE_SELF | FTYPE_BOTHGOOD | FTYPE_OGOOD)) || !up->userlevel)
#else
    /* 對方設我為好友可看見對方來源 */
    (!hide_from || sysop || (ftype & (FTYPE_SELF | FTYPE_BOTHGOOD | FTYPE_OGOOD)))
#endif
    ? up->from : "*", bmode(up, 0));


  if(cuser.ufo & UFO_IDLEONLY)
  {
process_idletime:
#ifdef DETAIL_IDLETIME
    if((diff = now - up->idle_time) >= 60)	/* 超過 60 秒才算閒置 */
    {
      if(diff >= 3600)	/* 閒置超過 1hr */
	prints("%2dh%02dm", diff / 3600, (diff % 3600) / 60);
      else
	prints("%2dm%02ds", diff / 60, diff % 60);
    }
#else
  ...
  {
    /* priority: 壽星>(極光沒老人這種權限)>標籤>站務 */
    if(up->status & STATUS_BIRTHDAY)
    {
      if(up->userlevel & PERM_ALLADMIN)	/* 生日的站務使用特殊顏色的"站務"二字 */
	outs(" \033[1;35m站誤\033[m");
      else
	outs(" \033[1;35m壽星\033[m");
    }
    else if (up->userlevel & PERM_LABEL)
    {
      attrset(up->label_attr);
      outc(' ');
      outs(up->label);
      attrset(7);
    }
    else if (up->userlevel & PERM_ALLADMIN)
      outs(" \033[1;32m站誤\033[m");
    else
      goto process_idletime;
  }
...
ulist_neck (XO *xo)
{
  ...
  if (!idlemode)
    ...
    prints(NECKER_ULIST_LABEL, d_cols >> 1, "", d_cols - (d_cols >> 1) + 4, pickup_ship ? "友誼" : "故鄉");
...
ulist_broadcast (XO *xo)
{
  ...
  if (bmw.caller >= 0)	/* bmw_edit() 中回答 Yes 要送出廣播 */
  {
...
ulist_write (XO *xo)
{
  ...
  {
    if (HAS_PERM(PERM_PAGE) || (up->userlevel & PERM_SpSYSOP))
      do_write(up);
  }
  return XO_NONE;
}
...
ulist_kick (XO *xo)
{
  if (HAS_PERM(PERM_ALLACCT))
  {
    ...
    if (pid = up->pid)
    {
      sprintf(buf, "Warning：即將踢 \033[1m%s\033[m 下線，請確認。 (Y/N) [N] ", up->userid);
      if (vans(buf) != 'y' || pid != up->pid)
	return XO_FOOT;
...
ulist_fromchange (XO *xo)
{
  ...
  if (vget(b_lines, 0, "請輸入新的故鄉：", buf, 19, GCARRY))
  {
    if (strcmp(buf, str))
    {
      strcpy(str, buf);
...
ulist_idchange (XO *xo)
{
  ...
  strcpy(buf, cutmp->userid);

  i = strlen(buf);
  while(i >= 0 && buf[i] != '.')
    i--;

  if(i >= 0) buf[i] = '\0';     /* 去除尾端上的點 */

  if(vget(b_lines, 0, "請輸入新的ＩＤ：", buf, IDLEN - 1, GCARRY))
  {
    if(strcmp(buf, cutmp->userid))
    {
      if(strcmp(buf, cuser.userid))
	strcat(buf, ".");               /* 多一個點作為分辨用 */

      strcpy(cutmp->userid, buf);
      return ulist_body(xo);
    }
  }
