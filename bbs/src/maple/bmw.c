...
extern int vget_BottomMode;	/* dust.100324: 解決水球蓋掉vget(b_lines, ...)的問題 */
...
can_override (UTMP *up)
{
  ...
  if (up->userno == cuser.userno)	/* 不能傳水球給自己(即使是分身) */
    return 0;                           /* 所以要是使用者編號重複就會有問題，請注意這個現象。 */
  ...
  else
  {
    ...
    for (ufo = bmw_locus - 1; ufo >= 0; ufo--)
    {
      ...
      if (bmw->caller == up - ushm->uslot && bmw->sender == up->userno)
	return 1;
    }
...
bmw_send (UTMP *callee, BMW *bmw)
{
  ...
  *mhead = *bmw;
  ...
  /* dust.101204: 但是現在的Maple3-itoc幾乎都是用inetd啟動的... */
  ushm->mbase = mhead;
  mslot[i] = mhead - mpool;
...
re_bmw:
  ...
  sprintf(fpath, "確定要送出《水球》給 %s 嗎(Y/N/C-修改)？[Y] ", up ? up->userid : "[廣播]");
...
can_reply (UTMP *uhead, int pos)
{
  ...
  if (bmw->caller != up - ushm->uslot || up->status & STATUS_REJECT)
    return NULL;
...
bmw_rqst (void)
{
  ...
  if (j)
  {
    ...
    /* dust.101026: 為防止印出水球使pfterm筆刷中途變色，記下現在的attr */
    attr = attrget();
    ...
    if(vget_BottomMode)
      ...
      move(b_lines, 0);
    clrtoeol();
    prints(BMW_FORMAT, mptr->userid, mptr->msg);

    /* dust.101108: 讓編輯器重繪foot */
    ve_sendrdrft();
...
bmw_item (int num, BMW *bmw)
{
  ...
  prints("%6d%c", num, tag_char(bmw->btime));

  if (bmw->sender == cuser.userno)	/* 送出的水球 */
    prints("\033[33m%-13s\033[36m", bmw->userid);
  else					/* 收到的水球 */
    prints("%-13s\033[32m", bmw->userid);

  len = strlen(bmw->msg);
  if(len > 46 + d_cols)
  {
    if(IS_ZHC_LO(bmw->msg, 44 + d_cols))
      len = 43;
    else
      len = 44;

    prints("%-*.*s", len + d_cols, len + d_cols, bmw->msg);
    prints("\033[34m%.*s", 46 - len, "...");
  }
  else
    prints("%-*s", 46 + d_cols, bmw->msg);

  ptime = localtime(&bmw->btime);
  prints("\033[m %02d/%02d %02d:%02d\n", ptime->tm_mon + 1, ptime->tm_mday, ptime->tm_hour, ptime->tm_min);
}
...
bmw_browse (XO *xo)
{
  ...
  sy = (y > b_lines / 2)? y - 4 : y + 1;

  getyx(&cy, NULL);
  scr_dump(&old_screen);
  grayout(0, y - 1, GRAYOUT_DARK);
  grayout(y + 1, b_lines, GRAYOUT_DARK);
  clrregion(sy, sy + 3);

  move(sy + 1, 1);
  prints(BMW_FORMAT, bmw->userid, bmw->msg);
  move(sy + 2, 1);
  prints("\033[1m時間：%s\033[m", Btime(&bmw->btime));
...
t_display (void)		/* itoc.020126: display FN_AMW */
{
  ...
  usr_fpath(fpath, cuser.userid, fn_amw);
  mo = more(fpath, MFST_NULL);	/* Thor.990204: 只要不是 XEASY 就可以 reload menu 了 */
  if(mo == -1)
  {
    vmsg("沒有水球紀錄");
    mo = XEASY;
...
re_ask:
#ifdef RETAIN_BMW
      op = vans("本次上站水球處理 (L)列出 (M)移到信箱 (R)保留 (C)清除 (S)存證？[R] ");
#else
      op = vans("本次上站水球處理 (L)列出 (M)移至信箱 (R)保留 (C)清除？[R] ");
#endif
    }
...
#ifdef RETAIN_BMW
    case 's':
      if (vans("存證是把水球紀錄寄給站長以檢舉使用者，您確定要存證嗎(Y/N)？[N] ") == 'y')
