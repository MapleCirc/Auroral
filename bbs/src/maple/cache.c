...
/* floatJ : note 雙向切換模式 */
static int note_number = -1;	/* 當前動態看板編號，<0為初始化用 */
static int note_direct = 0;	/* 跳躍值，可正可負 */
static int note_rand_flag = 0;	/* 是否要隨機跳躍 */
...
)
{
  ...
  {
    if(len >= 5)		/* 若 30 秒以後還沒換好片，可能是沒跑 camera，直接離開 */
      return;			/* dust.101001: 把timed out調成5秒。超過5秒絕對是camera炸了啊 */
  ...
  {
/* floatJ: 測試雙向切換，指導 by pioneerLike */
...
/* dust.091009: 效果改寫 */
    ...
    {
      if (note_number < 0)	/* initializing */
      {
        note_rand_flag = 1;	/* 其實應該要呼叫函數會比較適合 :p */
        note_number = 0;
      }

      len = fmax - FILM_MOVIE;	/* 以len儲存note的數量 */
      if(note_rand_flag)
      {
        note_direct = note_random(len - 1);
        note_rand_flag = 0;
      }
      note_number = (note_number + note_direct + len) % len;

      tag = FILM_MOVIE + note_number;
    }
  }
  else if(tag == FILM_OPENING0)	/* dust.101001: 不定數量的進站畫面 */
  {
    tag += rand() % fshm->opening_num;	/* dust.101005: 改用rand() */
  }
