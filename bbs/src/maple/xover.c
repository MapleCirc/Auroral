...
XO *
xo_get_post (		/* itoc.010910: 參考 xover.c xo_get()，為 XoPost 量身打造 */
    char *path,
    BRD *brd
)
{
  ...
  if (brd->btime < 0 || !rhs_unread(-1, brd->blast, 0) || 
  ...
  if (pos == 1)
  {
    ...
    if (read(fd, &chrono, sizeof(time_t)) == sizeof(time_t))
    {
      if (rhs_unread(-1, chrono, 0))	/* 若連第一篇也未讀，pos 調回去第一篇 */
	pos = 0;
    }
  }
  ...
  {
    for(pos = rec_num(path, sizeof(HDR))-1; pos>0; pos--)
    {
      if(lseek(fd, sizeof(HDR) * pos, SEEK_SET) < 0)
      {	/* lseek()失敗，我想應該不會發生吧。 */
        pos = XO_TAIL;
        break;
      }
...
int 
xo_rangedel (	/* itoc.031001: 區段刪除 */
    XO *xo,
    int size,
    int (*fchk)(char *),			/* 檢查區段中是否有被保護的記錄  0:刪除 1:保護 */
    void (*fdel)(XO *, char *)			/* 除了刪除記錄以外，還要做些什麼事 */
)
{
  char ans[8];
  int head, tail;
...
int 
xo_prune (		/* itoc.031003: 標籤刪除 */
    XO *xo,
    int size,
    int (*fvfy)(char *, int),			/* 檢查區段中是否有被保護的記錄  0:刪除 1:保護 */
    void (*fdel)(XO *, char *)			/* 除了刪除記錄以外，還要做些什麼事 */
)
{
  int fd, total, pos;
...
xo_tbf (
    XO *xo,
    int (*chk_readable)(HDR *)	/* 傳入NULL代表不需檢查 */
)
{
  char fpath[128], *dir;
  HDR *hdr, xhdr;
  int tag, locus, success;
  FILE *fp;
  ...
  do
  {
    ...
    if(!chk_readable || chk_readable(hdr))
    {
  ...
  {
     char buf[64];
     sprintf(buf, "成功\拷貝 %d 篇，共 %d 篇", success, tag);
     vmsg(buf);
  }
  else
  {
...
xo_forward (XO *xo)
{
  ...
  do
  {
    ...
    /* dust.101219: 現在 GEM_RESERVED 貌似沒在使用了，所以不納入限制 (而且這樣會卡到被鎖定的文章) */
	...
	if ((cc = rec_add(folder, &mhdr, sizeof(HDR))) < 0)
      ...
      {
	if(bsmtp(fpath, title, rcpt, MQ_READONLY) < 0)
	  break;
      }
    }
  } while (++locus < tag);
  ...
  if(method)
    vmsg(cc >= 0 ? msg_sent_ok : "部份信件無法寄達");
...
xo_thread (XO *xo, int op)
{
  ...
  {
    char *rs_menu[] = {"0首篇未讀", "1末篇已讀", "q取消", NULL};
    ...
    /* using krans(): makes life easier (笑) */
    chr = krans(b_lines, s_unread, rs_menu, "向前搜尋");
    if (chr == 'q')
      return XO_FOOT;
  ...
  for (;;)
  {
    ...
    if (op & RS_UNREAD)
    {
#define UNREAD_FUNC() (op & RS_BOARD ? rhs_unread(-1, hdr->chrono, hdr->stamp) : !(hdr->xmode & MAIL_READ))
      if (op & RS_FIRST)	/* 首篇未讀 */
      {
	if (UNREAD_FUNC())
	{
...
xover (int cmd)
{
  ...
  for (;;)
  {
    ...
    else if (cmd == KEY_END || cmd == '$')
    {
      ...
      {
	cmd = XO_MOVE;
	if(zone == XZ_POST && (cuser.ufo2 & UFO2_ENDJTNBP))
	  cmd += post_thelastNBpost(xo);
	else
	  cmd += xo->max - 1;
      }
    }
