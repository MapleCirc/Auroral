  ...
  MF *mf;
{
  ...
  if (mftype & MF_FOLDER)
  {
    ...
    prints("%6d%c  %s %s\n", num, mftype & MF_MARK ? ')' : ' ', "\033[1m◆\033[m", mf->title);
  }
  else if (mftype & MF_BOARD)
  {
    if ((bno = brd_bno(mf->xname)) >= 0)	/* dust.091227: 隱板不顯示標題 */
    {
	...
	prints("  %-13s    \033[36m<< 這個板是隱板 >>\033[m\n", mf->xname);
      }
    }
    else
  ...
  XO *xo;
{
  ...
  ans = vans("新增 (B)看板捷徑 (F)目錄 (G)精華區捷徑 (L)分隔線 [Q] ");
  ...
  XO *xo;
{
  ...
  /* itoc.010726.註解: 若是 MF_FOLDER，則換個檔名再貼上，一個目錄一個檔案 */
  /* itoc.010726.註解: 目錄複製貼上，裡面的東西並沒有貼上，懶得寫 recursive 的程式 :p */
  if (mf.mftype & MF_FOLDER)
  {
    if (mf_stamp(&mf) < 0)
    {
 ...
 ans = vans("您要把它放到哪個位置呢？ A)新增 I)插入 N)下一個 Q)取消 [A] ");
  ...
  XO *xo;
{
  ...
  if (bno >= 0)		/* itoc.010110: 應該不會 < 0 ? */
  {
    ...
    rhs_get(bshm->bcache[bno].bstamp, bno);
  ...
  XO *xo;
{
  ...
  BST *bstate = bshm->bstate;

  pos = xo->pos;
  mf = (MF *) xo_pool + (xo->pos - xo->top);
  max = xo->max;

  if(max > xo->top + XO_TALL)
    max = xo->top + XO_TALL;
  ...
  while (++pos < max)
  {
    ...
    /* 跳過分類與不能閱讀的看板 */
    if (bno >= 0 && (brd_bits[bno] & BRD_L_BIT))
    {
      /* itoc.010407: 改用最後一篇已讀/未讀來判斷 */
      if (rhs_unread(bcache[bno].bstamp, bstate[bno].blast[0], bstate[bno].blast[1]))
	return pos + XO_MOVE;
    }
  }

  /* dust.121001: 當本頁往下已經找不到未讀時，跳至下一頁開頭 */
  pos = xo->top + XO_TALL;
  if(pos >= xo->max)
    pos = 0;
