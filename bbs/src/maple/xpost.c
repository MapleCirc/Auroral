...
static int search_in_rfsmrk = 0;	/* 允許在看不到的密文中搜尋 */
...
)
{
  ...
  for (i = on; i < max; i++)
  {
    ...
    if (!search_in_rfsmrk && !chkrestrict(head))
      continue;
#endif
...
XoXuni (XO *xo)
{
  ...
  const char const *hotkeys = "s#t!xmirf";
  const char const *desc[] = {
    "完成標記", "特殊標記", "待砍", "鎖定", "加密", " m文", "轉信進來", "回文", "轉錄"
  };
  const usint sattr[] = {
    POST_DONE, POST_SPMS, POST_DELETE, POST_RESERVED, POST_RESTRICT, POST_MARKED, POST_INCOME, 0, 0
  };
  const char const *hint[] = {
    "含完成標記", "含特殊標記", "待砍", "鎖定", "加密", "mark", "轉信進來", "回覆", "轉錄"
  };
  ...
  outstr("                                    ");
  move(y++, 21);
  outstr("   ── 搜尋 \033[1;30m[模式:        ]\033[m ──   ");
  for(i = 0; i < max; i++)
  {
    move(y + i, 21);
    outstr("    ");
    if(i == cc)
      prints(   COLOR4  "  《  %-12s(%c)   》  \033[m", desc[i], hotkeys[i]);
    else
      prints("\033[30;47m      %-12s(%c)       \033[m", desc[i], hotkeys[i]);
    outstr("    ");
  }
  i += y;

  move(i++, 21);
  outstr("   ── \033[1;30mCtrl+G：切換搜尋模式 \033[m──   ");
  move(i, 21);
  outstr("                                    ");
  ...
  do
  {
    ...
    {
      move(y + oc, 25);
      prints("\033[30;47m      %-12s(%c)       \033[m", desc[oc], hotkeys[oc]);
      move(y + cc, 25);
      prints(   COLOR4  "  《  %-12s(%c)   》  \033[m", desc[cc], hotkeys[cc]);
    ...
    default:
      if(str = strchr(hotkeys, tolower(ch)))
      {
	cc = str - hotkeys;
	excute = 1;
      }
  ...
  sprintf(HintWord, "\033[1;33m所有 %s%s 文章\033[m", us_smode? "非" : "", hint[cc]);
  HintAuthor[0] = '\0';
  ...
  if(sattr[cc])
  {
    us_sattr = sattr[cc];
    search_in_rfsmrk = 1;
    return XoXpost(xo, NULL, 0, INT_MAX, filter_uni);
  }
  else
  {
    HDR hdr;
    ...
    search_in_rfsmrk = 0;
    return XoXpost(xo, &hdr, 0, INT_MAX, filter_unittl);
  }
}
...
#ifdef HAVE_REFUSEMARK
  if (!chkrestrict(hdr))	/* dust.100925: 允許找到密文 */
    return XO_NONE;
#endif
