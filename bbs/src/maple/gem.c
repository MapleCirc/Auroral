  ...
  int gtype;
{
  ...
  if (!gtype)
  {
    gtype = vans((level & GEM_X_BIT) ?
      ...
      "新增 (A)文章 (B)看板 (C)分類 (D)資料 (F)目錄 (L)分隔 (P)貼複 (Q)取消？[Q] " : 
      "新增 (A)文章 (F)目錄 (L)分隔 (P)貼複 (Q)取消？[Q] ");
  }
  ...
  XO *xo;
{
  ...
  if (xo->key & GEM_X_BIT)
    msg_sure = "請確定。(Y/N/C) [N] ";
  else
    msg_sure = msg_sure_ny;

  if(memcmp(fhdr, &mhdr, sizeof(HDR)))
  {
    switch (vans(msg_sure))
    {
    case 'y':
      modify = 1;
      break;
  ...
  XO *xo;
{
  ...
  {    
    ...
    if ((xmode = more(fpath, MFST_AUTO)) < 0)
      break;
  ...
  HDR *hdr;
{
  ...
  if (hdr->xmode & GEM_FOLDER)		/* 目錄/分類/看板 */
  {
  ...
  HDR *hdr;
{
  ...
  if ((gtype & GEM_RESTRICT) && !(check_key & GEM_M_BIT))
    return 0;
  ...
  int pos;		/* -1: 附加在最後  >=0: 貼上的位置 */
{
  ...
  if (xmode & GEM_FOLDER)	/* 目錄/分類 */
  {
    /* 在複製/貼上後一律變成目錄，因為分類是站長專用特殊用途的 */
  ...
  if (xmode & GEM_FOLDER)	/* 目錄/分類 */
  {
    /* 建立完自己這個目錄以後，再 recursive 地一層一層目錄進去一篇一篇另存新檔 */
    hdr_fpath(folder, srcDir, hdr);
