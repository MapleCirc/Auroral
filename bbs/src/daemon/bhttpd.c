...
/* dust.090316.註解: 
 * 因為我懶得同步修改 bhttpd.c 的設定，
 * 但是又不能放著它不管(這東西居然可以發文耶...)
 * 還好極光沒有開 bhttpd，所以決定從 Makefile 中拿掉它。
 * 如果要恢復的話請記得進行更新。 */
...
/* floatJ: 修改CSS */
    ...
    "<a href=/><img src=/img?site.gif border=0></a><br>\n"
    "<span class=\"linkstyle-nav\">"
    "<a href=javascript:history.back()>Back</a> /"
    "<a href=/class>Class</a> / "
    "<a href=/brdlist>Board</a> / "
    "<a href=/fvrlist>Favorite</a> / "
    "<a href=/mbox>Mail</a> / "
    "<a href=/usrlist>UserList</a> / "
    "<a href=telnet://tcirc.org>Telnet</a><br>\n"
    "</span>", fpw);
}
  ...
  int n;
{
  fprintf(fpw, "  <td>%d</td>\n"
    ...
    brd->class,
    /* floatJ: 轉信符號的色彩另外設定 :p */
 (brd->battr & BRD_NOTRAN)? "<span style=\"color: rgb(153, 153, 153);\">&nbsp;&nbsp;∫</span>"   : "<span style=\"color: rgb(0, 255, 0);\">∮</span>",
  ...
  Agent *ap;
{
  ...
  if (arg_analyze(4, 0, ap->urlp, &brdname, &title, &content, &end))
  {
	...
	fprintf(fp, "標題: %s\n時間: %s\n\n", hdr.title, Now());
	fprintf(fp, "%s\n", content);
	//fprintf(fp, ebs[0], ap->userid, getfromhost(&(ap->ip_addr)));
	fclose(fp);
  ...
  Agent *ap;
{
 ...
 out_reload(fpw, "您的信件發送失敗，也");  //許\是因為您尚未登入或是查無此使用者");
  return HS_OK;
}
