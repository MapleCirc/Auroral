...
#define BMLEN		47	/* Length of board managers */
...
#define LBLEN		4	/* Length of user label*/
...
#define MQ_READONLY	0x04	/* 寄出後不需刪除原檔案 */
#define MQ_NOATTACH	0x08	/* 退信通知中不附上原文 */
...
#define BRD_GOOD	0x01	/* 看板好友 */
#define	PAL_BAD		0x02	/* 好友 vs 壞人 */
#define PAL_MATE	0x08    /* 麻吉 */
...
/* ----------------------------------------------------- */
/* PAYCHECK : 支票 : 64 bytes				 */
/* ----------------------------------------------------- */
/* dust.080312: 現在是64位元的時代了(亂入)		 */
...
typedef struct
{
  ...
  int money;
  int gold;
  char reason[52];	/* 擴充至51字元，我就不信這樣還不夠用 */
}      PAYCHECK;
...
struct UTMP
{
  ...
  int talker;			/* who talk-to me ? */

  int mslot[BMW_PER_USER];
...
#endif
  uschar label_attr;
  char label[LBLEN + 1];	/* user's special label */
  char from[19];		/* 顯示在ulist上的故鄉名稱 */
  char fromhost[48];		/* remote host */
...
typedef struct BoardHeader
{
  ...
  char bvote;			/* 0:無投票/賭盤  1:有投票  2:有賭盤  3:兩種都有 */

  char btime;			/* -1:bpost/blast 需要更新 */
...
{
  int bpost;			/* 共有幾篇 */
  time_t blast[2];		/* 最後一篇的chrono, stamp */
  time_t blast_around[2];	/* 最後一篇的前一篇的chrono */
} BST;
...
typedef struct
{
  ...
  int bwlist_max;		/* 水桶名單中有幾人 */
  int bwlist_pool[PAL_MAX];	/* 所有被水桶的使用者的 userno */
  
  /* floatJ.091017: 板助功能 */
  int bvlist_max;               /* 板助名單中有幾人 */
  int bvlist_pool[PAL_MAX];     /* 所有板助的 userno */
}	BPAL;
...
#define NBRD_EXPIRE	0x00002	/* 連署過期 */
#define NBRD_START	0x00004	/* 連署開始 */
#define NBRD_END	0x00008	/* 連署結束(門檻已過) */
...
typedef struct
{
  ...
  int opening_num;
  char today[21];	/* dust.100312: 大小請維持在21不要動 否則選單有可能會爆炸... */
  char date[11];	/* dust.100312: 日期與星期幾 不受節日影響 */
  int otherside;	/* dust.100312: 節日位置特殊顯示 */
  MCD_info counter;
} FCACHE;
...
#define FILM_OPENING3	3
#define FILM_OPENING4	4
#define FILM_OPENING5	5
#define FILM_OPENING6	6
#define FILM_OPENING7	7
#define FILM_OPENING8	8
#define FILM_OPENING9	9
...
#define FILM_JUSTIFY	16	/* 身份認證的方法 */
#define FILM_REREG	17	/* 重新認證說明 */
#define FILM_EMAIL	18	/* 郵件信箱認證說明 */
#define FILM_NEWUSER	19	/* 新手上路須知 */
#define FILM_TRYOUT	20	/* 密碼錯誤 */
#define FILM_POST	21	/* 文章發表綱領 */
#define FILM_LOGINMSG   22	/* 登入歡迎訊息 (顯示在Login下面那行)*/
#define FILM_0ADMIN	23	/* (0)Admin專用note */
#define FILM_MOVIE	24	/* 動態看板 FILM_MOVIE 要放在最後面 */
