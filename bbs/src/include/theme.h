...
/* update :   /  /  				 	 */
/*-------------------------------------------------------*/
/* modified by R6 : 2007.5.26 : 修改全站配色及圖標       */
...
#define COLOR1		"\033[1;36;40m"	/* footer/feeter 的前段顏色 */
#define COLOR2		"\033[1;37;44m"	/* footer/feeter 的後段顏色 */
#define COLOR3		"\033[1;37;45m"	/* neck 的顏色 */
#define COLOR4		"\033[1;45m"	/* 光棒 的顏色 */
#define COLOR5		"\033[1;36;40m"	/* more 檔頭的標題顏色 */
#define COLOR6		"\033[1;37;44m"	/* more 檔頭的內容顏色 */
#define COLOR7		"\033[1;36m"	/* 作者在線上的顏色 */
#define COLOR8		"\033[;1;45m"	/* dust:給menu用的光棒色彩 */
...
#define COLOR_BRDMATE	"\033[0;35m"	/* 板伴 */
...
#define NECKER_ULIST		"\n" \
                        COLOR3 "  麻吉  代號         暱稱%*s                 %-*s               動態        閒置 \033[m"

#define NECKER_ULIST_LABEL	"\n" \
                        COLOR3 "  編號  代號         暱稱%*s                 %-*s               動態        標籤 \033[m"

#define NECKER_PAL	"[←]離開 [a]新增 [c]修改 [d]刪除 [m]寄信 [w]雪球 [s]整理 [→]查詢 [h]說明\n" \
			COLOR3 "  編號    代 號         友       誼%*s                                           \033[m"

#define NECKER_BPAL	"[←]離開 [a]新增 [c]修改 [d]刪除 [m]寄信 [w]雪球 [s]整理 [→]查詢 [h]說明\n" \
                        COLOR3 "  編號    代 號         註       解%*s                                           \033[m"

#define NECKER_ALOHA	"[←]離開 [a]新增 [d]刪除 [D]區段刪除 [m]寄信 [w]雪球 [s]重整 [f]引入 [h]說明\n" \
			COLOR3 "  編號   上 站 通 知 名 單%*s                                                    \033[m"

#define NECKER_VOTE	"[←]離開 [R]結果 [^P]舉行 [E]修改 [V]預覽 [^Q]改期 [o]名單 [h]說明\n" \
			COLOR3 "  編號      開票日   主辦人       投  票  宗  旨%*s                              \033[m"

#define NECKER_BMW	"[←]離開 [d]刪除 [D]區段刪除 [m]寄信 [M]儲存 [w]雪球 [s]更新 [→]查詢 [h]說明\n" \
			COLOR3 "  編號 代  號       內       容%*s                                          時間 \033[m"

#define NECKER_MF	"[←]離開 [→]進入 [^P]新增 [d]刪除 [c]切換 [C]複製 [^V]貼上 [m]移動 [h]說明\n" \
			COLOR3 "  %s   看  板       類別轉信看   板   標   題%*s              人氣 板    主%*s    \033[m"
...
#define NECKER_HELP	"[←]離開 [→]閱\讀 [/]搜尋(向下循環) [^P]新增 [d]刪除 [T]標題 [E]編輯 [m]移動\n" COLOR3 "  編號   檔 案          標       題%*s                                           \033[m"
...
#define FEETER_ULIST	\
COLOR1 " 網友列表 " COLOR2 " (f)好友 (t)聊天 (Q)查詢 (ad)交友 (m)寄信 (w)雪球 (s)更新 (TAB)切換 "
...
#define FEETER_BMW	\
COLOR1 " 雪球回顧 " COLOR2 " (d)刪除 (D)區段刪除 (m)寄信 (w)雪球 (^R)回訊 (^Q)查詢 (s)更新      "
...
/* dust.080513: 如果以後需要用到雙色字的技巧請用d_color.h */
#include "d_color.h"
#include "eb.rc"
...
#if 0
#define VMSG_NULL	"                           \033[1;33;44m ● 請按任意鍵繼續 ● \033[m"
#endif
#define VMSG_NULL	"\033[m                       \033[34m▌▌\033[37;44m \033[1;36m＊\033[;37;44m \033[1m請按任意鍵繼續\033[;37;44m \033[1;36m＊ \033[;30;44m▌▌\033[m"

#define ICON_UNREAD_BRD		"\033[1;33m★"		/* 未讀看板 */
#define ICON_CUNREAD_BRD	"\033[1;32m★\033[37m"	/* 未讀看板(推文) */
#define ICON_READ_BRD		"  "			/* 已讀看板 */

#define ICON_VOTGAM_BRD		"\033[1;33m賭\033[m"
#define ICON_GAMBLED_BRD	"\033[1;31m賭\033[m"	/* 舉行賭盤中的看板 */
#define ICON_VOTED_BRD		"\033[1;33m投\033[m"	/* 舉行投票中的看板 */
#define ICON_NOTRAN_BRD		"\033[1;30m∫\033[m"	 /* 不轉信板 */
#define ICON_TRAN_BRD		"\033[1m∮\033[m"			/* 轉信板 */
#define TOKEN_ZAP_BRD		'-'			/* zap 板 */
#define TOKEN_FRIEND_BRD	'#'			/* 好友板 */
#define TOKEN_SECRET_BRD	')'			/* 秘密板 */

#define RESTRICTED_TITLE	"\033[1;30m<< 模糊的雪花 >>\033[m"
