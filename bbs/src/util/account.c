  ...
  int (*attr)(int, int);
{
  ...
  if(stat(fpath, &st) < 0 || (count = st.st_size) <= 0)
    return 0;
  ...
  if (tmp = (PAL *) malloc(count))
  {
    count = read(fd, tmp, count) / sizeof(PAL);
    if (count > 0 && count <= PAL_MAX)
    {
      for(i = 0; i < count; i++)
      {
	pal_list[i] = attr(tmp[i].ftype, tmp[i].userno);
      }

      if (count > 1)
	qsort(pal_list, count, sizeof(int), int_cmp);
    }
...
#ifdef HAVE_MODERATED_BOARD
    brd_fpath(fpath, bcache[i].brdname, FN_PAL);
    pcache[i].pal_max = init_pallist(fpath, pcache[i].pal_spool, bpal_attr);

    brd_fpath(fpath, bcache[i].brdname, FN_BWLIST);
    pcache[i].bwlist_max = init_pallist(fpath, pcache[i].bwlist_pool, blist_attr);

    brd_fpath(fpath, bcache[i].brdname, FN_BVLIST);
    pcache[i].bvlist_max = init_pallist(fpath, pcache[i].bvlist_pool, blist_attr);
#endif
  }
}
  ...
  int verbose;
{
  ...
  for (;;)
  {
    ...
    *uptime = -1;
    ...
    time(uptime);
    if(verbose)
      fprintf(stderr, "[account]\treload bcache (%d boards)\n", bshm->number);
    return;
  }
}
  ...
  int mode;			/* 0:load  1:rename  2:unlink */
{
  ...
  if(stat(fnlog, &st) < 0)	/* dust.090228: 檔案不存在就不 keeplog */
    return;

  if(st.st_size <= 0)		/* dust.090228: 檔案是空的就不 keeplog */
    return;
  ...
  VCH *vch;
{
  ...
  *fname = 'Z';
  ...
  if (!(brd->battr & BRD_NOVOTE))
  {
    ...
    keeplogV(fpath, BN_RECORD, buf, 0, 0);
  }

  keeplogV(fpath, bid, "[記錄] 選情報導", 2 ,1);
  ...
  BRD *brd;
{
  ...
  *fname++ = '/';
  ...
  do
  {
    if (head->vclose < now)
    {
      if(head->vgamble == ' ')	/* 賭盤不讓 account 開 */
      {
	strcpy(fname, head->xname);
	draw_vote(brd, buf, head);
	head->chrono = 0;
      }
      else
	alive++;
  ...
  char *argv[];
{
  ...
  /* dust.120813: 如果有參數代表不是在 crontab 中跑的，不輸出訊息 */
  init_bshm(argc != 1? 0 : 1);
  ...
  if (ntime.tm_hour == 0)
  {
    ...
    sprintf(title, "%s寄信失敗記錄", date);
    keeplog(FN_RUN_MAILFAIL_LOG, BN_SECURITY, title, 2);
    ...
    sprintf(title, "%s站務紀錄", date);
    keeplog(FN_RUN_ADMIN, BN_SECURITY, title, 2);

    sprintf(title, "%ssysop活動紀錄", date);
    keeplog(FN_RUN_SYSOP, BN_SECURITY, title, 2);

    sprintf(title, "%s幻符.行走於天上的電研神.召喚紀錄", date);
    keeplog(FN_RUN_CIRCGOD, BN_SECURITY, title, 2);

#ifdef SP_SEND_MONEY
    sprintf(title, "%sSYSOP發錢紀錄", date); 
    keeplog(FN_RUN_BONUS_LOG, BN_SECURITY, title, 2); 
#endif
    ...
    sprintf(title, "%s被清除的使用者列表", date);
    keeplog(FN_RUN_REAPER, BN_SECURITY, title, 2);

    sprintf(title, "%s特殊權限使用者統計", date);
    keeplog(FN_RUN_SPPERM, BN_SECURITY, title, 2);

    sprintf(title, "%s變動的板主名單", date);
    keeplog(FN_RUN_MODBM, BN_SECURITY, title, 2);
