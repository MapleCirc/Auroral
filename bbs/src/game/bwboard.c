...
do_init()
{
  ...
  *cmdBuf = 0;
  ...
  sprintf(buf, "☆%s(%d戰%d勝) vs ★%s(%d戰%d勝) \033[m", 
    cuser.userid, myTotal, myWin, mateid, yourTotal, yourWin);
  ...
  int sock, later;
{
  ...
  if (!later)
  {
    char *rule_set[] = {"1黑白棋", "2五子棋", "3圍棋", "4軍棋", "5暗棋", "q取消", NULL};

    /* ask for which rule set */
    /* assume: peer won't send char until setup */
    c = krans(b_lines, "1q", rule_set, "想下哪種棋？");
    if (c >= '1' && c <= '5')
      c -= '1';
    else
      c = -1;
  ...
  else
  {
    ...
    outz("★ 對方正在選擇對奕項目中，請稍候 \033[5m...\033[m");
    ...
    while(1)
    {
      ...
      if(ch == I_OTHERDATA)
        ...
        send(cfd, &cfd, 1, 0);	/* trigger I_OTHERDATA */
    }    
