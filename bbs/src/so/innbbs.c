...
/* floatJ: 如果autofillin就複製字串(偷懶) */
  if (autofillin==1 && (brd = ask_board(nf.board, BRD_L_BIT, NULL)))
  {
    char tmp_nfboard[20];

    autofillin=0;
    strcpy(nf.path,"Group.NCTU");
    strcpy(nf.charset, "big5");

    str_lower(tmp_nfboard,nf.board);
    sprintf(nf.newsgroup, "group.tcirc.%s", tmp_nfboard);
...
a_innbbs()
{
  ...
  */
  ...
  do
  {
    ...
    case 'a':
    case Ctrl('P'):
      /* floatJ:如果按的是'a'，就用auto fill in*/
      if (ch=='a') autofillin = 1;
